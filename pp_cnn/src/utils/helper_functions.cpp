...
void encryptImages(const vector<vector<float>>& origin_images, Ciphertext3D& target_packed_images, const size_t& begin_idx, const size_t& end_idx) {
  ...
  const size_t pixels_per_channel = rows * cols;
  vector<double> pixels_in_slots(gTool.slot_count(), 0);
  Plaintext plaintext_packed_pixels;

  if (size_t target_images_count = end_idx - begin_idx; target_images_count < gTool.slot_count()) {  // fill slots if size of target images < slot_count
    size_t origin_images_size = origin_images.size();
    size_t fill_images_count  = gTool.slot_count() - target_images_count;
    for (size_t ch = 0; ch < channels; ++ch) {
      for (size_t row = 0; row < rows; ++row) {
        for (size_t col = 0; col < cols; ++col) {
          ...
          gTool.encoder()->encode(pixels_in_slots, gTool.scale_param(), plaintext_packed_pixels);
          gTool.encryptor()->encrypt(plaintext_packed_pixels, target_packed_images[row][col][ch]);
