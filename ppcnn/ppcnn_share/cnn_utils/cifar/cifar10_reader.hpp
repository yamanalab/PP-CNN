 ...
 */
template <template <typename...> class Container, typename Image, typename Label = uint8_t>
CIFAR10_dataset<Container, Image, Label> read_dataset_direct(const std::string& folder, std::size_t training_limit = 0, std::size_t test_limit = 0) {
    CIFAR10_dataset<Container, Image, Label> dataset;

    read_training(folder, training_limit, dataset.training_images, dataset.training_labels, [] { return Image(3 * 32 * 32); });
    read_test(folder, test_limit, dataset.test_images, dataset.test_labels, [] { return Image(3 * 32 * 32); });
 ...
 */
template <template <typename...> class Container = std::vector, template <typename...> class Sub = std::vector, typename Pixel = uint8_t, typename Label = uint8_t>
CIFAR10_dataset<Container, Sub<Pixel>, Label> read_dataset(const std::string& folder, std::size_t training_limit = 0, std::size_t test_limit = 0) {
    return read_dataset_direct<Container, Sub<Pixel>, Label>(folder, training_limit, test_limit);
}
